...
#### 5.2.2 RS485模块连接
- **RTS引脚**：连接到ESP8266的GPIO13引脚（方向控制）
- **RX-1引脚**：连接到ESP8266 GPIO12引脚（发送数据）
- **TX-0引脚**：连接到ESP8266 GPIO14引脚（接收数据）
...
RX-1 | 接收数据 | RX-1 | 连接到ESP8266 GPIO14引脚 |
TX-0 | 发送数据 | TX-0 | 连接到ESP8266 GPIO12引脚 |
RTS | 方向控制 | RTS | 连接到ESP8266 GPIO13引脚 |
A | RS485+ | A | RS485总线正极 |
B | RS485- | B | RS485总线负极 |
G | RS485地 | G | RS485总线地线 |

### 13.2 方向控制实现
- **控制信号**：RTS引脚（GPIO13引脚）控制RS485收发方向
- **发送模式**：RTS=高电平（启用发送器）
- **接收模式**：RTS=低电平（启用接收器）
